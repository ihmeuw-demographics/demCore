...
#' @param id_cols character vector of column names in dt that, along with age_start
...
calc_qx <- function(dt, age_start, age_end, id_cols = c()) {
  ...
  # Example: if there are 5 age groups within, the resulting dataset should
  #  be 1/5th the size of the original
  nrow_collapse <- nrow(dt)
  if (nrow_collapse == nrow_master) {
    stop("You have specified an id variable that is unique within age --
         re-check your variables")
  }
  if (nrow_collapse * n_ages != nrow_master) {
    stop("The input dataset is not square (e.g. not all required age groups
         are present for all combinations of id_cols)")
  }
