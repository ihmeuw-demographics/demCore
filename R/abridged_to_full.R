...
#'   lx spline
#'
#' @param dt \[`data.table()`\]\cr with variables 'age_start', 'age_end', all
#'   `id_cols`, 'qx', 'ax'
#' @param id_cols \[`character()`\]\cr variables that uniquely identify rows,
#'   must include 'age_start' and 'age_end'.
#' @param regression_fits \[`data.table()`\]\cr with variables from
...
#' @param lx_spline_start_age \[`integer(1)`\]\cr age (inclusive) to start using lx
...
#' @param preserve_input_ax_ages \[`integer()`\]\cr ages to preserve the input ax
#'   values for. This is typically the first age group 0-1 and the terminal age
#'   group 110+. Corresponds to 'age_start' column.
#'
#' @return \[`data.table()`\]\cr with columns id_cols, age, qx, ax
#'
