...
```{r example_lt, echo = F}
data("exampleLT")
dt <- lifetable(exampleLT, id_cols = c("age_start", "age_end"))
dt <- dt[, .SD, .SDcols = c("age_start", "age_end", "deaths", "population",
...
```{r mx_calc}
data("exampleLT")
dt <- exampleLT[, c("age_start", "age_end", "deaths", "population")]
...
Note that we can use `demUtils::gen_length` to add the `age_length` column
...
The `mx_ax_to_qx` combines the equation for $q_x$ and the requirement of
terminal $q_x$ equalling one by setting $q_x = 1$ if `age_length = Inf`.
...
You can also solve for $q_x$ under the assumption of contant mortality rate
...
```{r lx_calc}
qx_to_lx(dt, id_cols = c("age_start", "age_end"))
```
...
To calculate $d_x$, use `lx_to_dx`:

```{r dx_calc}
lx_to_dx(dt, id_cols = c("age_start", "age_end"))
```
