...
```{r ax_calc}
dt[, ax := mx_to_ax(mx = mx, length = age_length)]
```
...
```{r qx_calc1}
dt[, qx := mx_ax_to_qx(mx = mx, ax = ax, length = age_length)]

# set qx = 1 and recalculate ax for terminal age group
dt[age == max(dt$age), qx := 1]
dt[age == max(dt$age), ax := mx_qx_to_ax(mx = mx, qx = 1, length = 30)]
```
...
```{r qx_calc2, eval = F}
dt[, qx_compare := mx_to_qx(mx = mx, length = age_length)]
```
