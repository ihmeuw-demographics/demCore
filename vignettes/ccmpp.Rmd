...
The demographic balancing equation can be extended to be by age and sex, this is called the cohort component method of population projection (CCMPP).
It is the most common method for projecting populations forward in time.

It takes as input age and sex specific estimates of the baseline population, survivorship ratios, net migration proportion, age-specific fertility rates and sex-ratio at birth to produce projected population estimates starting from the baseline.
...
The lexis diagrams in the following sections were created using `LexisPlotR` which I highly recommend.
...
When modeling actual populations immigration and emigration needs to be accounted for.
These migrants are also at risk of dying and giving birth in their new populations but we don't know exactly when in the interval they migrated.
...
For the youngest age group we need to calculate the number of births occuring in the interval and then use the survivorship ratios to calculate the proportion that survive to the next time point.

**Example equation**:
$$\begin{aligned} {_1}N_{0}(2018) =& \cdot B[2017, 2018] \cdot \frac{{_1}L_{0}[2017, 2018]}{l_0[2017, 2018]} \\ B[2017, 2018] =& \sum_{x=0}^{A} \frac{1}{2} \cdot  {_1}F_x \cdot \frac{{_1}N^F_{x+1}(t) + {_1}N^F_x(t) \cdot \frac{{_1}L_{x+1}[t, t+1]}{{_1}L_{x}[t, t+1]}}{2} \end{aligned}$$

**General equation**:
$$\begin{aligned} {_n}N_{0}(t + n) =& \cdot B[t, t + n] \cdot \frac{{_n}L_{0}[t, t + n]}{l_0[t, t + n]} \\ B[t, t + n] =& \sum_{x=0}^{A} \frac{n}{2} \cdot {_n}F_x[t, t + n] \cdot \frac{{_n}N^F_{x}(t) + {_n}N^F_{x - n}(t) \cdot \frac{{_n}L_{x}[t, t + n]}{{_n}L_{x - n}[t, t + n]}}{2} \end{aligned}$$
...
We can write all these tedious ccmpp equations in matrix notation which helps to speed up computation and can lead to other useful relationships from matrix algebra.
